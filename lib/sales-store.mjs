/**
 * Repository for revenue data: per-booking sale amounts, walk-in sales,
 * monthly ad spend, and the rows feeding the Google Ads offline-conversion
 * CSV. Follows the same conventions as lib/store.mjs / special-orders-store.
 *
...
/**
 * @returns { ok: true, sale } | { ok: false, error: 'INVALID_AMOUNT'|'INVALID_DATE' }
 */
export async function createWalkinSale({ amountCents, note, saleDate } = {}, idGenerator) {
  const amount = normalizeAmountCents(amountCents);
  if (amount === null) return { ok: false, error: 'INVALID_AMOUNT' };
  if (saleDate !== undefined && saleDate !== null && saleDate !== '' && !DATE_RE.test(String(saleDate))) {
  ...
  const db = getDb();
  await db.execute({
    sql: `INSERT INTO walkin_sales (id, amount_cents, note, sale_date, created_at)
          VALUES (?, ?, ?, ?, ?)`,
    args: [sale.id, sale.amountCents, sale.note, sale.saleDate, sale.createdAt],
  });
  return { ok: true, sale };
}
...
/**
 * @param {object} opts
 * @param {string} [opts.month] - 'YYYY-MM' filter
 * @param {number} [opts.limit] - max rows (default 200, clamped 1-1000)
 */
export async function listWalkinSales({ month, limit = 200 } = {}) {
  const db = getDb();
  const where = [];
  const args = [];
  if (month && MONTH_RE.test(month)) {
    where.push('sale_date LIKE ?');
    args.push(`${month}-%`);
  }
  const whereSql = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  args.push(Math.max(1, Math.min(1000, Number(limit) || 200)));
  const result = await db.execute({
    sql: `SELECT * FROM walkin_sales ${whereSql} ORDER BY sale_date DESC, created_at DESC LIMIT ?`,
    args,
  });
  return result.rows.map(rowToWalkin);
}

/**
 * @returns { ok: true } | { ok: false, error: 'INVALID_AMOUNT'|'INVALID_DATE'|'NO_FIELDS'|'NOT_FOUND' }
 */
export async function updateWalkinSale(id, { amountCents, note, saleDate } = {}) {
  const sets = [];
  const args = [];
  ...
  if (note !== undefined) {
    sets.push('note = ?');
    args.push(typeof note === 'string' ? note.trim().slice(0, 200) : '');
  }
  if (saleDate !== undefined) {
    if (!DATE_RE.test(String(saleDate))) return { ok: false, error: 'INVALID_DATE' };
    sets.push('sale_date = ?');
    args.push(saleDate);
  }
  ...
  const db = getDb();
  const result = await db.execute({
    sql: `UPDATE walkin_sales SET ${sets.join(', ')} WHERE id = ?`,
    args,
  });
  if (result.rowsAffected === 0) return { ok: false, error: 'NOT_FOUND' };
  return { ok: true };
...
export async function deleteWalkinSale(id) {
  const db = getDb();
  const result = await db.execute({
    sql: 'DELETE FROM walkin_sales WHERE id = ?',
    args: [id],
  });
  if (result.rowsAffected === 0) return { ok: false, error: 'NOT_FOUND' };
  return { ok: true };
 ...
 * customers = appointments with a sale > 0 + walk-in sales (each walk-in row
 * is one customer). aov/cac are null when their denominator is 0.
 * @returns [{ month, spendCents, bookingsCreated, gclidBookings, completed,
...
export async function getRevenueStats(months = 12) {
  ...
  const [created, completed, apptSales, walkins, spend] = await Promise.all([
    ...
    db.execute(`SELECT substr(slot_date, 1, 7) AS month, COUNT(*) AS completed
                FROM bookings WHERE staff_status = 'completed' GROUP BY month`),
    db.execute(`SELECT substr(sale_recorded_at, 1, 7) AS month,
                       COUNT(*) AS customers,
                       SUM(sale_amount_cents) AS revenue
                FROM bookings WHERE sale_amount_cents > 0 GROUP BY month`),
    db.execute(`SELECT substr(sale_date, 1, 7) AS month,
                       COUNT(*) AS customers,
                       SUM(amount_cents) AS revenue
                FROM walkin_sales GROUP BY month`),
  ...
  for (const r of completed.rows) {
    monthRow(r.month).completed = Number(r.completed);
  }
