 ...
 *
 * Bookings additionally start life as 'new' (written directly by
 * createBooking) — the unacknowledged state that drives the dashboard's red
 * badge. 'new' is intentionally NOT in this list: it is never staff-assignable,
 * staff clear it by moving the booking forward to one of the statuses below.
 *
 * confirmed — staff has seen and acknowledged the appointment
