 ...
 *
 * completed — the customer attended
...
/**
 * Purchase-amount filters for the staff dashboard, keyed by the value of the
 * `?purchase=` query param. Fixed SQL fragments (no interpolated user input).
 * `unrecorded` surfaces completed appointments whose sale hasn't been logged
 * yet — the "go enter the number" work queue.
 */
export const PURCHASE_FILTERS = {
  bought:     'sale_amount_cents > 0',
  '200plus':  'sale_amount_cents >= 20000',
  '400plus':  'sale_amount_cents >= 40000',
  none:       'sale_amount_cents = 0',
  unrecorded: "sale_amount_cents IS NULL AND staff_status = 'completed'",
};
 ...
 * @param {string} [opts.purchase] - a PURCHASE_FILTERS key, or 'all'
 ...
 */
export async function listBookings({ status, audience, purchase, search, limit = 200 } = {}) {
  const db = getDb();
  const where = [];
  const args = [];
  ...
  if (purchase && PURCHASE_FILTERS[purchase]) {
    where.push(PURCHASE_FILTERS[purchase]);
  }
