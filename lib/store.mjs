...
function rowToIntake(r) {
  return {
    ...
    sheetsStatus: r.sheets_status,
    sheetsDetail: r.sheets_detail,
    ip: r.ip,
    userAgent: r.user_agent,
    createdAt: r.created_at,
  };
...
export async function createIntake(record, idGenerator) {
  ...
  await db.execute({
    sql: `INSERT INTO intakes (
      ...
      suit_size, suit_color, tailoring_notes, need_by_date,
      tailor_status, review_email_status, sheets_status,
...
export async function updateIntakeSheetsStatus(id, status, detail) {
  const db = getDb();
  const result = await db.execute({
    sql: 'UPDATE intakes SET sheets_status = ?, sheets_detail = ? WHERE id = ?',
 ...
 * @param {object} opts
 * @param {string} [opts.status]  - filter by tailor_status: 'pending'|'sent'|'back'|'picked_up'|'open' (open = not picked_up)
...
export async function listIntakes({ status, search, limit = 200 } = {}) {
  ...
  if (status === 'open') {
    where.push("tailor_status != 'picked_up'");
