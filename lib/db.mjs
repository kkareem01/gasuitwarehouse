...
const DEFAULT_OFFER = {
  id:                 'default-free-tie',
  name:               'Free Silk Tie',
  item_description:   'A hand-finished silk tie from our shop. Yours to keep — no purchase necessary.',
  retail_value_cents: 4500,
  week_start:         '2024-01-01',
  week_end:           '2099-12-31',
  redemption_cap:     999999,
};

async function ensureDefaultOffer() {
  const db = getDb();
  const rs = await db.execute('SELECT 1 FROM lead_magnet_offers WHERE active = 1 LIMIT 1');
