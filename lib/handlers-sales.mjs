 ...
 *
 * Auth model:
 * - handleAdminRecordBookingSale: staff-token guarded (writes against
 *   specific bookings from the appointments dashboard).
 * - ad-spend / ad-spend-set / revenue-stats: intentionally unguarded
 *   (owner request) — aggregate numbers only, no customer PII, so the
 *   stats page loads without a token. Inputs stay strictly validated.
