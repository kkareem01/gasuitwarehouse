...
export function nurtureT3Email({ booking, offer, code, businessName, businessAddress, businessPhone, confirmUrl }) {
  const dateLong = formatLongDate(booking.slot.date);
  const time = formatTime12h(booking.slot.time);
...
export function nurtureT1Email({ booking, offer, code, businessName, businessAddress, businessPhone, confirmUrl }) {
  const dateLong = formatLongDate(booking.slot.date);
  const time = formatTime12h(booking.slot.time);
  const subject = `Tomorrow at ${time} — your ${offer.name}`;
  const safeFirst = escapeHtml(booking.customer.firstName);
  const html =
    WRAPPER_OPEN +
    `
...
export function nurtureDayOfEmail({ booking, offer, code, businessName, businessAddress, businessPhone }) {
  const time = formatTime12h(booking.slot.time);
