...
export async function sendEmail({ to, from, subject, html, text, attachments, replyTo }) {
...
export function giftCodeReadyEmail({ firstName, offer, code, expiresAt, businessName, businessAddress, businessPhone }) {
  ...
  const html =
    WRAPPER_OPEN +
    `
    <h1 style="font-family:Georgia,serif;font-size:24px;font-weight:600;color:#14213D;margin:0 0 12px 0;">${escapeHtml(firstName)}, your free gift is reserved.</h1>
    <p style="margin:0 0 20px 0;font-size:16px;">Show this code in store — your <strong>${escapeHtml(offer.name)}</strong> is yours, no purchase necessary.</p>
  ...
  const text =
    `${firstName}, your free gift is reserved.\n\n` +
    `Show this code in store: ${code}\n` +
...
export function redemptionCodeEmail({ booking, offer, code, expiresAt, businessName, businessAddress, businessPhone, confirmUrl }) {
  const dateLong = formatLongDate(booking.slot.date);
  const time = formatTime12h(booking.slot.time);
  const subject = `Your ${offer.name} is reserved — pickup ${dateLong} at ${time}`;
  const expiresOn = formatExpires(expiresAt);
