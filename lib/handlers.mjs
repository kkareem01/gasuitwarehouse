 ...
 * every /api/admin/* handler (here and in lib/handlers-sales.mjs), so a
 * request that reaches a handler without the token gets a uniform 401.
 */
...
export async function handleAdminListBookings(req, res) {
  ...
  const purchase = url.searchParams.get('purchase') || 'all';
  const search = url.searchParams.get('q') || '';
  const limit = parseInt(url.searchParams.get('limit') || '200', 10) || 200;
  const format = url.searchParams.get('format') || 'json';

  try {
    const bookings = await listBookings({ status, audience, purchase, search, limit });
    if (format === 'csv') {
