...
async function fireEmails(booking, cfg, issuedCode = null, issuedOffer = null) {
  ...
  const siteUrl = process.env.SITE_URL || 'https://www.gasuitwarehouse.com';
...
/**
 * Staff dashboard auth: Bearer STAFF_TOKEN, constant-time compare. Now used
 * only by the gift-code redeem endpoints (lookup-code, redeem), which can move
 * money. Everything the staff CRM dashboard (/staff/intakes) calls — intakes,
 * special orders, bookings — plus the aggregate stats endpoints is
 * deliberately unguarded (owner request, 2026-07-17) so staff pages load
 * without pasting a token. Those pages rely on noindex + unguessable URLs.
 */
export function verifyStaffAuth(req) { return verifyBearer(req, 'STAFF_TOKEN'); }
...
/* Token check intentionally removed (owner request): aggregate counts only,
   no customer PII. Page is noindex + unguessable URL. */
...
// Token checks intentionally removed from the intake/special-order/booking
// handlers below (owner request): the /staff/intakes dashboard must load
