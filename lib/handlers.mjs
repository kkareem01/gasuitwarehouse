...
import {
  ...
  issueCodeForBooking,
  hasRecentUnredeemedCodeForEmail,
} from './offers.mjs';
...
const STYLING_FALLBACK_REDIRECT = '/styling-session/?from=disqualified';
const STYLING_EXHAUSTED_REDIRECT = '/styling-session/?from=exhausted';
...
export async function handleLeadMagnetOptIn(req, res) {
  ...
  // Disqualified path: still capture the lead, fire consolation email, redirect.
  if (qualifierAnswer === 'no') {
    const result = await createLead(
      {
        audience: 'general',
        firstName,
        ...
        qualifierAnswer,
        qualified: false,
        source: 'styling-session-fallback',
        ip,
        userAgent,
      },
      newLeadId
    );
    if (!result.ok) return sendJson(res, 500, { ok: false, error: 'SAVE_FAILED' });
    log.info(`disqualified opt-in ${result.lead.id}`);
    fireConsolationEmail({ firstName, email }).catch((e) =>
      log.error('consolation email crash', e?.message || e)
    );
    return sendJson(res, 200, {
      ok: true,
      qualified: false,
      redirect: STYLING_FALLBACK_REDIRECT,
    });
  }
  ...
  if (!offer) {
    return sendJson(res, 200, {
      ...
      qualified: true,
      exhausted: true,
      redirect: STYLING_EXHAUSTED_REDIRECT,
    });
  }

  // Soft duplicate guard: same email already has an unredeemed code recently.
  const dup = await hasRecentUnredeemedCodeForEmail(email).catch(() => false);
  if (dup) {
  ...
  log.info(`qualified opt-in ${leadResult.lead.id} offer=${offer.id} code=${code}`);
  return sendJson(res, 200, {
    ok: true,
    qualified: true,
    leadId: leadResult.lead.id,
...
async function fireConsolationEmail({ firstName, email }) {
  const businessName = process.env.BUSINESS_NAME || 'GA Suit Warehouse';
  const businessPhone = process.env.BUSINESS_PHONE || '+14705957775';
  const fromEmail = process.env.FROM_EMAIL || 'bookings@example.com';
  const siteUrl = process.env.SITE_URL || 'http://localhost:3000';
  const stylingSessionUrl = `${siteUrl}/styling-session/`;
  const tpl = disqualifiedConsolationEmail({
    firstName,
    businessName,
    businessPhone,
    stylingSessionUrl,
...
function verifyCronAuth(req) {
  ...
  // Constant-time-ish compare. Buffers must be same length (checked above).
  let diff = 0;
