...
import { validateBookingPayload, validateLeadPayload, validateLeadMagnetPayload, validateIntakePayload } from './validate.mjs';
import {
...
import {
  ...
  createIntake,
  updateIntakeSheetsStatus,
  listIntakes,
  updateIntakeTailorStatus,
} from './store.mjs';
import { mirrorIntakeToSheet, isSheetsConfigured } from './sheets.mjs';
import {
  getActiveOffer,
  findOfferById,
  reserveOfferSlot,
...
export async function handleCreateIntake(req, res) {
  ...
  // Mirror to Google Sheet — best-effort, non-fatal. Awaited so the row's
  // sheets_status reflects reality before we respond, and so the function
  // doesn't terminate before the fetch resolves on Vercel.
  if (isSheetsConfigured()) {
    const mirror = await mirrorIntakeToSheet(result.intake);
    await updateIntakeSheetsStatus(
      result.intake.id,
      mirror.ok ? 'mirrored' : 'failed',
      mirror.ok ? null : mirror.detail,
    ).catch((e) => log.warn(`could not update sheets status: ${e?.message || e}`));
  }
...
function intakesToCsv(rows) {
  const headers = [
    ...
    'suit_size', 'suit_color', 'tailoring_notes', 'need_by_date', 'tailor_status',
  ];
  const escape = (v) => {
  ...
  for (const r of rows) {
    lines.push([
      ...
      r.suitSize, r.suitColor, r.tailoringNotes, r.needByDate, r.tailorStatus,
    ].map(escape).join(','));
  }
  return lines.join('\r\n');
}
