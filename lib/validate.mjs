...
export function validateIntakePayload(body, nowMs = Date.now()) {
  ...
  const sizeRaw = typeof body.suitSize === 'string' ? body.suitSize.trim() : '';
  if (sizeRaw.length === 0) errors.push('Suit size is required.');
  else if (sizeRaw.length > 16) errors.push('Suit size is too long.');

  const colorRaw = typeof body.suitColor === 'string' ? body.suitColor.trim() : '';
  if (colorRaw.length === 0) errors.push('Suit color is required.');
  else if (colorRaw.length > 32) errors.push('Suit color is too long.');

  const tailRaw = typeof body.tailoringNotes === 'string' ? body.tailoringNotes.trim() : '';
  if (tailRaw.length === 0) errors.push('What needs to be tailored is required.');
